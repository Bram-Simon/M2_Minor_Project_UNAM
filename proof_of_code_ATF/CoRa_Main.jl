...
# Define iARG with appropriate values
iARG = (
    mm = "ATFv2",
    ex = "ATF",
    pp = :mY,
    an = "ExSSs",  # Change to your analysis type, e.g., "ExDyn", "DYms", "OptDY"
	ax = :mY # Provide an appropriate value for ax
)
            ...
            while(rtol >= 1e-24)
                ...
                if(abs(mm.outFB(ssR) - mm.outNF(soR)) > 1e-4)
                    ...
                    if(rtol < 1e-24)
                        ...
                        println(vcat(pert.p,i,[p[i] for i in mm.odeFB.sys.ps],mm.outFB(ssR),mm.outNF(soR)))
